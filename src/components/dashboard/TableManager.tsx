...
export default function TableManager({
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto relative text-[#f0ece4] min-h-screen">
      ...
      {/* ── Page Header ────────────────────────────────────────────────── */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[rgba(255,255,255,0.06)] pb-5 no-print">
        <div className="flex items-center gap-3.5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 no-print">
        {[
          ...
          { title: "Total Seating", val: `${totalCapacity} Guests`, icon: Users, color: "text-emerald-400" },
          { title: "Average Table Size", val: `${averageSeats} Seats`, icon: Sparkles, color: "text-blue-400" },
          { title: "Occupied Tables", val: Object.values(positions).filter(p => p.status === "OCCUPIED").length, icon: ShieldCheck, color: "text-rose-400" }
        ].map((item, idx) => (
          <div key={idx} className="bg-[#111] border border-[rgba(255,255,255,0.04)] rounded-2xl p-4 flex items-center gap-3.5 shadow-md">
            ...
            <div>
              <p className="text-[9px] text-[#6f695f] uppercase tracking-wider font-extrabold">{item.title}</p>
              <p className="text-[15px] font-extrabold text-white mt-0.5 leading-none">{item.val}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <AnimatePresence>
        {adding && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -15 }}
            className="bg-[#111] border border-[#f97316]/30 rounded-2xl p-5 shadow-2xl no-print space-y-4"
          >
            <div className="flex items-center gap-2 mb-1">
              <div className="w-1 h-5 bg-[#f97316] rounded-full" />
              <p className="text-[13px] font-bold text-white uppercase tracking-wider">Register Table</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-[#9a9488] text-[10px] font-bold uppercase tracking-wider block">Table Label *</Label>
                <Input
                  autoFocus
                  value={newTable.name}
                  ...
                  className="h-10 bg-[#161616] border-[rgba(255,255,255,0.08)] text-[#f0ece4] placeholder-[#444] focus:border-[#f97316] text-[13px] rounded-xl outline-none"
                />
              </div>

              <div className="space-y-1.5">
                <Label className="text-[#9a9488] text-[10px] font-bold uppercase tracking-wider block">Seating Capacity</Label>
                
                {/* Custom Capacity Segmented Quick Toggles */}
                <div className="grid grid-cols-5 gap-1.5 bg-[#141414] p-1 rounded-xl border border-[rgba(255,255,255,0.06)]">
                  {["2", "4", "6", "8"].map((cap) => (
                    <button
                      key={cap}
                      type="button"
                  ...
                  <input
                    ...
                    className="w-full text-center bg-transparent border-0 text-[12px] font-mono font-bold text-white placeholder-[#5a5650] focus:outline-none focus:ring-0"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2.5 pt-1.5">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-[#161616] border border-[rgba(255,255,255,0.08)] text-[#9a9488] hover:text-white hover:bg-[#222] transition-colors text-[12px] font-bold"
              >
                Cancel
              </button>
            </div>
      ...
      {tables.length === 0 && !adding && (
        <div className="text-center py-24 bg-[#111] border border-[rgba(255,255,255,0.06)] rounded-2xl shadow-xl no-print">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#161616] border border-[rgba(255,255,255,0.06)] mx-auto mb-4">
            <QrCode className="size-6 text-[#5a5650]" />
          </div>
          <p className="text-[16px] font-bold text-white">No Active Tables</p>
          <p className="text-[12px] text-[#9a9488] mt-1 max-w-xs mx-auto">
      ...
      {viewMode === "floor" && tables.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 no-print">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-[#0c0c0c] border border-[rgba(255,255,255,0.05)] rounded-2xl p-4.5 space-y-4">
              <div className="border-b border-[rgba(255,255,255,0.04)] pb-3">
                ...
                <p className="text-[10px] text-[#6f695f] mt-0.5">Click to place tables onto the layout floor</p>
              </div>

              {unplacedTables.length === 0 ? (
                <div className="py-8 text-center text-xs text-[#555] bg-[#111] border border-dashed border-[rgba(255,255,255,0.04)] rounded-xl">
                ...
                <div className="space-y-2 max-h-[220px] overflow-y-auto scrollbar-thin">
                  {unplacedTables.map(t => (
                    <button
                      ...
                      className="w-full p-2.5 bg-[#141414] border border-[rgba(255,255,255,0.04)] hover:border-[#f97316]/40 text-left rounded-xl transition-all flex items-center justify-between group"
                    >
                      <div>
                        <div className="text-xs font-bold text-white">{t.name}</div>
                        <div className="text-[9px] text-[#6f695f]">{t.capacity || 4} seats</div>
                      </div>
            ...
            <div className="bg-[#0c0c0c] border border-[rgba(255,255,255,0.05)] rounded-2xl p-4.5 space-y-4">
              <div className="border-b border-[rgba(255,255,255,0.04)] pb-3">
                ...
                <p className="text-[10px] text-[#6f695f] mt-0.5">Place plants, partitions, doors, or bar counters</p>
              </div>

              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: "Partition Wall", type: "wall" as const, color: "bg-[#222] border-[#444]" },
                  { label: "Bar Counter", type: "bar" as const, color: "bg-amber-900/10 border-amber-950/20 text-amber-500" },
                  { label: "Entry Door", type: "door" as const, color: "bg-blue-950/15 border-blue-900/25 text-blue-400" },
                  { label: "Visual Plant", type: "plant" as const, color: "bg-emerald-950/15 border-emerald-900/25 text-emerald-400" }
                ].map((item, idx) => (
                  <button
                    key={idx}
            ...
            {/* Controls Info Deck */}
            <div className="bg-[#0c0c0c] border border-[rgba(255,255,255,0.05)] p-4 rounded-2xl space-y-2.5 text-[11px] text-[#9a9488]">
              <div className="font-bold text-white text-xs flex items-center gap-1"><Info className="size-3.5 text-[#f97316]" /> Help Desk & Guidelines</div>
              <ul className="list-disc pl-4 space-y-1.5 text-[#6f695f]">
              ...
              <button
                ...
                className="w-full mt-2.5 py-2 rounded-xl bg-rose-500/10 border border-rose-500/25 text-rose-400 hover:bg-rose-500/20 text-xs font-bold transition-all flex items-center justify-center gap-1"
              >
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {/* Canvas wrapper card */}
            <div className="bg-[#0c0c0c] border border-[rgba(255,255,255,0.05)] rounded-3xl p-4 md:p-6 flex flex-col justify-between">
              
              {/* Canvas header controls */}
              <div className="flex items-center justify-between border-b border-[rgba(255,255,255,0.04)] pb-3 mb-4">
                <span className="text-xs text-[#9a9488] font-semibold flex items-center gap-1.5">
                  <Compass className="size-4 text-[#f97316] animate-pulse" />
                  Visual Layout Workspace Grid ({placedTables.length} Tables Active)
                </span>
                <span className="text-[10px] text-[#6f695f]">Drag elements freely</span>
              </div>
              ...
              >
                ...
                {placedTables.map((table) => {
                  ...
                  const statusColors = {
                    FREE: "border-emerald-500/40 bg-emerald-500/10 text-emerald-400 shadow-emerald-500/5",
                    RESERVED: "border-amber-500/40 bg-amber-500/10 text-amber-400 shadow-amber-500/5",
                  ...
                  return (
                    <motion.div
                      ...
                      className={`absolute w-24 h-24 rounded-2xl border-2 flex flex-col justify-between p-2.5 cursor-grab active:cursor-grabbing transition-shadow hover:shadow-lg ${
                        ...
                        isSelected ? "ring-2 ring-[#f97316] ring-offset-2 ring-offset-[#070707]" : ""
                      }`}
                    >
                  ...
                  return (
                    <motion.div
                      ...
                      className={`absolute cursor-grab active:cursor-grabbing p-3 flex flex-col items-center justify-center rounded-xl text-center select-none shadow-md ${
                        decor.type === "wall" ? "w-28 h-6 bg-[#222] border border-[#444] text-[#888]" :
                        decor.type === "bar" ? "w-32 h-14 bg-amber-900/10 border border-amber-900/30 text-amber-500" :
                        decor.type === "door" ? "w-16 h-10 bg-blue-950/20 border border-blue-900/30 text-blue-400" :
                        "w-12 h-12 bg-emerald-950/25 border border-emerald-900/30 text-emerald-400 rounded-full"
                      } ${
                        isSelected ? "ring-2 ring-[#f97316] ring-offset-2 ring-offset-[#070707]" : ""
                      }`}
                    >
              ...
              <AnimatePresence>
                {selectedPlanItem && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 15 }}
                    className="bg-[#111] border border-[rgba(255,255,255,0.06)] rounded-2xl p-4.5 mt-4 space-y-4"
                  >
                    {selectedPlanItem.type === "table" ? (() => {
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="text-sm font-bold text-white flex items-center gap-2">
                                {table.name}
                                <span className="text-[10px] text-[#6f695f]">({table.capacity || 4} Capacity)</span>
                              </div>
                              <p className="text-[10px] text-[#9a9488]">Change seating states or remove from plan workspace</p>
                            </div>
                          </div>

                          <div className="flex flex-wrap items-center gap-2">
                            {/* Live Status triggers */}
                            <div className="flex rounded-xl bg-[#161616] p-1 border border-[rgba(255,255,255,0.04)]">
                            ...
                            <button
                              ...
                              className="px-3.5 py-1.5 bg-[#1a1a1a] border border-[rgba(255,255,255,0.06)] hover:text-white rounded-lg text-xs font-bold text-[#f0ece4] transition-all flex items-center gap-1"
                            >
                            ...
                            <button
                              ...
                              className="px-3.5 py-1.5 bg-rose-500/10 border border-rose-500/25 text-rose-400 hover:bg-rose-500/20 rounded-lg text-xs font-bold transition-all"
                            >
                              Remove from Floor
                            </button>
                            ...
                            <button
                              ...
                              className="w-8 h-8 rounded-lg bg-[#222] hover:bg-[#333] flex items-center justify-center text-white"
                            >
                              <X className="size-4" />
                            </button>
                          </div>
                      ...
                      return (
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-[#161616] flex items-center justify-center text-amber-500">
                              <Sparkles className="size-5" />
                            </div>
                            <div>
                              <div className="text-sm font-bold text-white uppercase">{decor.type} element</div>
                              <p className="text-[10px] text-[#6f695f]">Interactive decoration item</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2.5">
                            <button
                              ...
                              className="px-4 py-1.5 bg-rose-500/10 border border-rose-500/25 text-rose-400 hover:bg-rose-500/20 rounded-lg text-xs font-bold transition-all"
                            >
                              Remove Item
                            </button>
                            <button
            ...
            {tables.map((table, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-[#111] border border-[rgba(255,255,255,0.06)] hover:border-[#f97316]/30 rounded-2xl overflow-hidden shadow-lg transition-all duration-300 group flex flex-col justify-between"
                >
                  {/* Card Header Info */}
                  <div className="p-4 flex items-center justify-between border-b border-[rgba(255,255,255,0.05)] bg-[#131313]/30">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="font-extrabold text-[13px] text-white truncate leading-tight">{table.name}</div>
                    </div>
                    
                    {table.capacity && (
                      <span className="flex items-center gap-1 text-[10px] text-[#5a5650] font-mono font-bold bg-[#161616] px-2 py-0.5 rounded-full border border-[rgba(255,255,255,0.04)]">
                  ...
                  <div className="p-4 flex flex-col items-center justify-center">
                    {hasQr ? (
                      ...
                      <div className="w-[140px] aspect-square bg-[#161616] border border-dashed border-[rgba(255,255,255,0.08)] rounded-xl animate-pulse flex items-center justify-center text-[#333]">
                        <QrCode className="size-8" />
                      </div>
                    )}
                    
                    <span className="text-[9px] text-[#5a5650] hover:text-[#f97316] text-center font-mono mt-3 truncate w-full select-all tracking-tight" title={`Table Link: /${restaurantSlug}/${table.qrToken}`}>
                      {`/${restaurantSlug}/${table.qrToken}`}
                    </span>
                  </div>

                  {/* Card Actions Panel */}
                  <div className="p-3 bg-[#131313]/30 border-t border-[rgba(255,255,255,0.04)] flex gap-2">
                    <button
                      onClick={() => openCustomizer(table)}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-[#161616] border border-[rgba(255,255,255,0.06)] hover:bg-[#222] text-[#9a9488] hover:text-white text-[11px] font-bold transition-all"
                    >
                      <Printer className="size-3" /> Branded Flyer
                    </button>
                    <button
                      onClick={() => downloadQR(table)}
                      className="w-9 h-9 rounded-xl bg-[#161616] border border-[rgba(255,255,255,0.06)] hover:bg-[#222] text-[#9a9488] hover:text-white flex items-center justify-center transition-all"
